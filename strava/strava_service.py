...
logging.config.fileConfig(pathlib.Path.cwd().parent/'logging_file_config.conf')
...
def refresh_access_token(refresh_token=sensitive_info_api(sensitive_info_file)[0],
                         ...
                         client_secret=sensitive_info_api(sensitive_info_file)[2]):
    try:
        result=client.refresh_access_token(client_id=client_id, client_secret=client_secret, refresh_token=refresh_token)
        ...
        with open(pathlib.Path('/Users/maximkalinchenko/Desktop/Garmin_Strava_project/sensitive_info.json'),
            ...
            json.dump(data, file)
            file.truncate()
            logger.info(f"New access token: {access_token}, new refresh token is {refresh_token}")
